...
// Responsibility: Dot entity model and factory helpers for spawning dots within bounds.
export class Dot implements DotState {
  ...
  // Attempt to create a dot that is within bounds and not overlapping the player.
  ...
  static spawnRandom(id: string, bounds: { width: number; height: number }, player?: PlayerState, radius = 8): Dot {
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      // If player provided, ensure not overlapping the player's bbox
      if (player) {
        const halfW = player.width / 2 + radius + 2; // small padding
        const halfH = player.height / 2 + radius + 2;
        if (x > player.x - halfW && x < player.x + halfW && y > player.y - halfH && y < player.y + halfH) {
          continue; // collision with player area, retry
        }
      }
      return new Dot(id, x, y, radius, 1);
    }
    // Fallback: place at center if sampling fails
